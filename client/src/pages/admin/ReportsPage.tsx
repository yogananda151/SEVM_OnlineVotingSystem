...
import { FileText, Table2, BarChart3, ClipboardList, Users, Vote, Loader2 } from 'lucide-react';
...
export const ReportsPage: React.FC = () => {
  ...
  /** Wraps any report download with toast feedback and per-card loading state (#14) */
  const withToast = (reportKey: string, fn: () => Promise<void>) => async () => {
    ...
    try {
      ...
      const msg =
        ...
        'Download failed. Please try again or check the server logs.';
      toast.error(msg);
    } finally {
  ...
  const reportCards = [
    {
      key: 'election-summary',
      title: 'Election Summary',
      description: 'Complete election report including constituency-wise candidate list',
      icon: Vote, color: 'text-blue-400', bg: 'bg-blue-500/10', format: 'PDF',
      action: withToast('election-summary', () =>
        selectedElection
          ? reportService.downloadElectionSummaryPDF(Number(selectedElection))
          : Promise.reject({ response: { data: { message: 'Please select an election first.' } } }),
      ),
    },
    {
      ...
      icon: BarChart3, color: 'text-emerald-400', bg: 'bg-emerald-500/10', format: 'Excel',
      action: withToast('election-results', () =>
        selectedElection
          ? reportService.downloadResultsExcel(Number(selectedElection))
          : Promise.reject({ response: { data: { message: 'Please select an election first.' } } }),
      ),
    },
    {
      ...
      icon: Users, color: 'text-amber-400', bg: 'bg-amber-500/10', format: 'Excel',
      action: withToast('voters-list', () =>
        selectedStation
          ? reportService.downloadVotersExcel(Number(selectedStation))
          : Promise.reject({ response: { data: { message: 'Please select a polling station first.' } } }),
      ),
    },
    {
      key: 'audit-log',
      ...
      icon: ClipboardList, color: 'text-purple-400', bg: 'bg-purple-500/10', format: 'PDF',
      action: withToast('audit-log', () => reportService.downloadAuditLogPDF()),
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        {reportCards.map((card) => {
          const isLoading = loadingReport === card.key;
          return (
            <div key={card.title} className="card p-6 flex items-start gap-4 hover:border-slate-600/50 transition-all duration-200">
              ...
              <div className="flex-1">
                ...
                <button
                  onClick={card.action}
                  disabled={isLoading}
                  className="btn-secondary mt-4 text-sm py-2 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {isLoading
                    ? <><Loader2 size={14} className="animate-spin" /> Generating…</>
                    : <>{card.format === 'PDF' ? <FileText size={14} /> : <Table2 size={14} />} Download {card.format}</>
                  }
                </button>
              </div>
            </div>
