...
class VoteRepository {
    async castVote(data) {
        ...
        return database_1.prisma.$transaction(async (tx) => {
            ...
            if (voter.deletedAt)
                throw new error_middleware_1.AppError('Voter record not found.', 404);
            if (!voter.isActive)
                throw new error_middleware_1.AppError('Voter account is inactive.', 403);
            ...
            if (!election)
                throw new error_middleware_1.AppError('No active election found for this constituency.', 400);
            ...
            if (existingStatus?.hasVoted) {
                throw new error_middleware_1.AppError('Voter has already cast their vote in this election.', 409);
            }
            ...
            if (candidate.electionId !== election.id)
                throw new error_middleware_1.AppError('Candidate does not belong to the current active election.', 400);
    ...
    async getVvpat(referenceNumber, pollingStationId) {
        ...
        const directVvpat = await database_1.prisma.digitalVVPAT.findFirst({
        ...
        const voteByRef = await database_1.prisma.vote.findFirst({
            where: {
        ...
        //    for a new election.
        const voter = await database_1.prisma.voter.findFirst({
            where: {
                voterId: cleanRef,
                deletedAt: null,
                ...(pollingStationId ? { pollingStationId } : {}),
        ...
        if (voter) {
            ...
            if (hasActiveElection) {
                throw new error_middleware_1.AppError(`Voter "${voter.fullName}" (${voter.voterId}) has not yet cast a vote in the current election. Please cast your ballot at the voting machine to generate a digital VVPAT slip.`, 404);
            }
            throw new error_middleware_1.AppError(`Voter "${voter.fullName}" (${voter.voterId}) has not cast a vote in any active election.`, 404);
        }
        return null;
    }
    ...
    async getDashboardStats() {
        ...
        if (activeElection) {
            ...
            const activeElectionLinks = await database_1.prisma.electionConstituency.findMany({
